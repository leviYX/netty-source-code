...
[![CompletableFuture](https://github.com/Acyco/CompletableFuture/raw/master/images/CompletableFuture.png)](https://github.com/Acyco/CompletableFuture/blob/master/images/CompletableFuture.png)
...
这些方法是一个`Consumer<? super T>`，它可以对异步任务的执行结果进行消费使用，方法返回CompletableFuture
...
前面我们已经知道，通过`thenApply(Function<T,R>)`对链式操作中的上一个异步任务的结果进行转换，返回一个新的结果; 通过`thenAccpet(Consumer<T>)`对链式操作中的上一个异步任务的结果进行消费，不返回新结果;
...
以上程序一种可能的运行结果（需要多运行几次），因为异步调度有可能会启动新线程，有时候就会是同一个，但是你不指定异步就一定是一个：
...
此外，如果将Executor传递给`thenApplyAsync()`回调，则该回调的异步任务将在从Excutor的线程池中获取的线程中执行。此时就一定不是同一个了，因为fj有优化，这种自定义线程池没有这个优化。
...
> 这里我解释一下这里的问题:他是嵌套了方法，我们为了方便看，可以不抽取方法，写的直观一点。

~~~java
public static void main(String[] args) throws ExecutionException, InterruptedException {
    CompletableFuture<CompletableFuture<String[]>> future = CompletableFuture.supplyAsync(() -> {
        return CommonUtils.readFile(Constant.prefix + "filter_words.txt");
    }).thenApply(content -> {
    ...
    //  此时future就是一个返回值，而且其内部类型还是CompletableFuture<String[]>，发生了嵌套，thenApply内部返回的就是一个CompletableFuture
    // 而不是我们要的那个处理结果值，但这里很怪，像是故意制造问题来说明现象。你完全可以直接处理逻辑返回，没必要在里面再套一层CompletableFuture。他主要是想说明组合多个completableFuture的使用
}
~~~
...
如果我们想连接（编排）两个依赖关系的异步任务（CompletableFuture对象），请使用thenCompose()方法，所以他的意义就是连接两个异步future。这就是他上面强行举那个例子的意义所在。
...
让我们看看使用 CompletableFuture 是不是执行的更有效率
...
注意的是， 以上给的仅仅是参考值，实际开发环境复杂，CPU不可能理想化的给哪个进程使用，需要酌情分析。
...
[![比价](https://github.com/Acyco/CompletableFuture/raw/master/images/image1.png)](https://github.com/Acyco/CompletableFuture/blob/master/images/image1.png)
